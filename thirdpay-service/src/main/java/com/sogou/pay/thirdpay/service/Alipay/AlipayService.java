...
public class AlipayService implements ThirdpayService {
  ...
  static {
    ...
    CHECK_TYPE.put(CheckType.ALL,
        StringUtils.join(
            Arrays.asList(AlipayTradeCode.TRADE_CODE_PAY.getValue(), AlipayTradeCode.TRADE_CODE_TRANSFER.getValue(),
                AlipayTradeCode.TRADE_CODE_CHARGE.getValue(), AlipayTradeCode.TRADE_CODE_CASH.getValue()),
            ','));
  ...
  public ResultMap<?> queryRefundOrder(PMap<String, ?> params) throws ServiceException {
    ...
    if (StringUtils.isEmpty(returnRefundId) || StringUtils.isEmpty(returnRefundMon)
        || StringUtils.isEmpty(refundIsSuccess)) {
      log.error("[queryRefundOrder] response error, request={}, response={}", requestPMap, refundResult);
      return ResultMap.build(ResultStatus.THIRD_RESPONSE_PARAM_ERROR);
    }
  ...
  private ResultMap<?> validateAndParseMessage(String message) {
    ...
    //支付数据解析
    accountLogVoList = doc
        .selectNodes("/alipay/response/account_page_query_result/account_log_list/AccountQueryAccountLogVO");
    for (Node accountLogVo : accountLogVoList) {

      OutCheckRecord record = new OutCheckRecord();
      //第三方账户余额
      item = accountLogVo.selectSingleNode("balance");
      if (StringUtils.isEmpty(item.getText())) record.setBalance(BigDecimal.valueOf(0));
      else record.setBalance(BigDecimal.valueOf(Double.parseDouble(item.getText())));
      //手续费
      item = accountLogVo.selectSingleNode("service_fee");
      if (StringUtils.isEmpty(item.getText())) record.setCommssionFee(BigDecimal.valueOf(0));
      else record.setCommssionFee(BigDecimal.valueOf(Double.parseDouble(item.getText())));
      //交易金额
      item = accountLogVo.selectSingleNode("income");
      if (StringUtils.isEmpty(item.getText())) record.setMoney(BigDecimal.valueOf(0));
      else record.setMoney(BigDecimal.valueOf(Double.parseDouble(item.getText())));
      //退款金额
      item = accountLogVo.selectSingleNode("outcome");
      if (!StringUtils.isEmpty(item.getText())) record.setMoney(BigDecimal.valueOf(Double.parseDouble(item.getText())));
      //我方订单号
      ...
      if (list == null) log.error("[validateAndParseMessage] response error, trans_code_msg={}", item.getText());
  ...
  public ResultMap<?> prepareTransferInfo(PMap<String, ?> params) throws ServiceException {
    ...
    for (int i = 0, size = records.size(); i < size; i++) {
      ...
      sb.append(StringUtils.join(Arrays.asList(record.getSerialNumber(), record.getAccountNo(), record.getAccountName(),
