    ...
    public ResultMap queryRefund(QueryRefundModel queryRefundModel) {
        ...
        try {
            ...
            if (null == payOrderInfo) {
                logger.error("Query Refund Request ,Does Not Exist Order,Params :" + JsonUtil.beanToJson(queryRefundModel));
                return ResultMap.build(ResultStatus.Q_RF_ORDERID_ERROR);
            }
            ...
            if (CollectionUtils.isEmpty(refundInfoList)) {
                logger.error("Query Refund Request ,Does Not Exist Order Refund,Params :" + JsonUtil.beanToJson(queryRefundModel));
                return ResultMap.build(ResultStatus.Q_RF_ORDERNOTREFUND_ERROR);
            }
            // 4.检查退款单状态是否有退款成功的
            ...
            if (null == app) {
                logger.error("Query Refund Request ,Does Not Exist AppInfo,Params :" + JsonUtil.beanToJson(queryRefundModel));
            ...
            if (null == agencyMerchant) {
                //支付机构商户不存在
                logger.error("Query Refund Request ,Does Not Exist Merchant Info,Params :" + JsonUtil.beanToJson(queryRefundModel));
            ...
            if (null == agencyInfo) {
                //支付机构不存在
                logger.error("Query Refund Request ,Does Not Exist Agency Info,Params :" + JsonUtil.beanToJson(queryRefundModel));
            ...
            logger.error("Query Refund request error,params :" + JsonUtil.beanToJson(queryRefundModel) + "error：" + e);
