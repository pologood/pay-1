...
public class PayNotifyManagerImpl implements PayNotifyManager {
    ...
    public ResultMap doProcess(PayNotifyModel payNotifyModel) {
        ...
        try {
            LOGGER.info("pay notify biz check begin for:" + payNotifyModel.toString());
            //将回调ID插入排重表
            ...
            if (0 == flag){
                ...
                LOGGER.info("Pay notify biz check success for:" + payNotifyModel.toString());
                
                /******写入队列******/
                writeQue(payOrderInfo,payNotifyModel);
                LOGGER.info("pay notify writeQue success for:" + payNotifyModel.toString());
            } else {
                //业务勾兑
            ...
            //支付成功，重复通知，丢弃该通知
            LOGGER.error("payReqId:" + payReqId + " part repeat pay");
            result.withError(ResultStatus.RES_DETAIL_ALREADY_EXIST);
        } catch (Exception e) {
            LOGGER.error("Pay notify error" + e.getMessage());
    ...
    public int validateOrder(PayNotifyModel payNotifyModel, PayOrderInfo payOrderInfo, PayReqDetail payReqDetail) throws Exception {
        ...
        if (trueMoney.compareTo(payNotifyModel.getTrueMoney()) != 0) { //BigDecimal不能用equals
            LOGGER.error("payReqId:" + payReqDetail.getPayDetailId()
                    + "agency money is not equal our money.agency money is " + payNotifyModel.getTrueMoney()
                    + ",our money is " + trueMoney);
            throw new Exception("biz check money error");
        }
        return 0;
    }
    ...
    public PMap<String, BigDecimal> computerFee(PayNotifyModel payNotifyModel, PayOrderInfo payOrderInfo, PayReqDetail payReqDetail) throws Exception {
        ...
        LOGGER.info("computer payFee result is {}", feeMap.get("fee"), feeMap.get("feeRate"));
        return feeMap;
    }
     ...
     * @param payOrderInfo
     */
    public void writeQue(PayOrderInfo payOrderInfo,PayNotifyModel patyNotifyModel) {
    ...
    public ResultMap<Map> getNotifyMap(
        ...
        if(!Result.isSuccess(result)){
            LOGGER.error("【同步通知】获取同步通知参数失败！" + result.getStatus().getMessage());
