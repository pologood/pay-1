...
public class PayTransferQueryManagerImpl implements PayTransferQueryManager {
  ...
  public Result doProcess(PayTransferBatch payTransferBatch) {
    ...
    try {
      ...
      //查询交易概要信息 数据提交
      String agentResultStr = HttpUtil.sendPost(PayConfig.payTransferHost, agentRequestStr);
      ...
      if (updatePayTransferBatch != null) {
        ...
        //查询交易明细信息 数据提交
        String resultDetailStr = HttpUtil.sendPost(PayConfig.payTransferHost, agentDetailRequestStr);
  ...
  protected String getAgentRequestStr(PayTransferBatch payTransferBatch) {
    ...
    // 构造直接代发代扣的请求报文
    XmlPacket xmlPkt = new XmlPacket("GetAgentInfo", PayConfig.payTransferLgnName);
    ...
    queryMap.put("BGNDAT", PayConfig.payTransferQueryDate);
    //结束日期
    queryMap.put("ENDDAT", PayConfig.payTransferQueryDate);
    //业务参考号
  ...
  protected String getAgentDetailRequestStr(String reqNbr) {
    ...
    XmlPacket xmlPkt = new XmlPacket("GetAgentDetail", PayConfig.payTransferLgnName);
    Map queryMap = new Properties();
    //流程实例号
