...
import com.sogou.pay.service.payment.PayCheckDayLogService;
...
/**
 * @Author qibaichao
 * @ClassName PayClearDayLogServiceImpl
 * @Date 2015年2月16日
 * @Description:
 */
@Service
public class PayCheckDayLogServiceImpl implements PayCheckDayLogService {

    private static final Logger logger = LoggerFactory.getLogger(PayCheckDayLogServiceImpl.class);
    ...
    public boolean isSuccessDownload(String checkDate, String agencyCode) throws ServiceException {
        ...
        // 如果状态不是下载完成状态，直接返回
        if (status != OperationLogStatus.SUCCESS.value()) {
            logger.warn(String
